...
def test_load_data_invalid_input(hiv_model, column_mapping):
    ...
    with pytest.raises(
        ValueError, match="Data must be a file path or pandas DataFrame"
    ):
...
def test_simulate_with_custom_initial_conditions(
    hiv_model, sample_data, column_mapping
):
...
def test_simulate_with_uncertainty_without_calibration(
    hiv_model, sample_data, column_mapping
):
...
def test_simulate_with_uncertainty_after_calibration(
    hiv_model, sample_data, column_mapping
):

...
class HIVOpen:
    ...
    def load_data(self, data, columnDictionary):
        ...
        self.I0 = initial_row[self.columns["new_infections"]]   # Initial infectious population
        self.S0 = self.N - self.I0  # Initial susceptible population
        self.E0 = self.S0 * 0.001   # Initial exposed population, exposure rate is around 0.1%
        self.R0 = initial_row[self.columns["total_hiv"]] * initial_row[self.columns["viral_suppression"]]   # Recovered population (virally suppressed)
        self.D0 = initial_row[self.columns["deaths_hiv"]]   # Initial deaths from HIV

        # Initialize demographic and ART-related parameters
        self.b = initial_row[self.columns["number_of_births"]]      # Annual number of births
        self.d = initial_row[self.columns["natural_death_rate"]] / 100000       # Death rate per individual
        self.art = initial_row[self.columns["viral_suppression"]]       # Initial ART (viral suppression) proportion
    ...
    def simulate(self, years, initial_conditions=None, params=None):
        ...
        def seirs(t, y):
            ...
            dS_dt = self.b - self.beta * S * I / N - self.mu * S + self.gamma * R       # Susceptible
            dE_dt = self.beta * S * I / N - self.sigma * E - self.mu * E                # Exposed
            dI_dt = self.sigma * E - effective_nu * I - self.mu * I - self.delta * I    # Infectious
    ...
    def calibrate_parameters(self, num_bootstrap=100):
        ...
        def loss_function(params, data):
            ...
            observed_virally_suppressed = (
                data[self.columns["viral_suppression"]] * data[self.columns["total_hiv"]]
            ).values
            ...
            all_simulated_deaths.append(simulated["Deaths (HIV)"].diff().fillna(0).values)

            # Calculate MSE for infections, recovered, and deaths
            mse_infections = mean_squared_error(observed_infections, simulated["Infectious"].values) / observed_infections.mean()
            mse_recovered = mean_squared_error(observed_virally_suppressed, simulated["Recovered (ART)"].values) / observed_virally_suppressed.mean()
            mse_deaths = mean_squared_error(observed_deaths, simulated["Deaths (HIV)"].diff().fillna(0).values) / observed_deaths.mean()
        ...
        observed_virally_suppressed = (
            self.data[self.columns["viral_suppression"]] * self.data[self.columns["total_hiv"]]
        ).values
        ...
        plt.plot(years, observed_infections/1000, label="Observed New Infections", marker='o')
        plt.plot(years, mean_simulated_infections/1000, label="Mean Simulated New Infections", linestyle="--")
        plt.xlabel("Year")
        ...
        plt.plot(years, mean_simulated_deaths/1000, label="Mean Simulated Deaths", linestyle="--")
        plt.xlabel("Year")
        ...
        plt.figure(figsize=(12, 6))
        plt.plot(years, observed_virally_suppressed/1000, label="Observed Recovered", marker='o')
        plt.plot(years, mean_simulated_recovered/1000, label="Mean Simulated Recovered", linestyle="--")
        plt.xlabel("Year")
    ...
    def simulate_with_uncertainty(self, years):
        ...
        for i, compartment in enumerate(compartments):
            plt.plot(years, mean_results[:, i]/1000, label=f"Mean {compartment}", color=colors[i])
            plt.fill_between(years, lower_ci[:, i]/1000, upper_ci[:, i]/1000, color=colors[i], alpha=0.2)
